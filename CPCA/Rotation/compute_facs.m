function [T, PR, VR, UR] = compute_facs( P, U, V,nd,tsum,nr,nc, Gheader, rotation_specs, pop, GAtyp )
...
%  Cor = [];
  ...
  if ( rotation_specs.parameters.HRF )	
      ...
      if isempty( shapes ) && isempty(rotation_specs.defaults.T_mat)
        ermsg = 'Unable to perform hrfmax rotation as there are no estimated HRF shapes to process with.';
...
%        if ( iter*size(shapes,2)~=size(P,1) )
        if ( size(shapes,2)~=Gheader.bins ) && strcmpi( rotation_specs.method, 'hrfmax' )
          ermsg = 'Unable to perform hrf based rotation as the given shapes are of the wrong dimension.';
          show_message( 'Unable to rotate',ermsg );
          return;
        end;
      end;
...
%     end; 
  ...
  if ( rotation_specs.parameters.iterations )	
    if ( rotation_specs.defaults.iterations <= 0 )	
      ermsg = 'Unable to perform rotation as the number of iterations is zero (or less).';
      show_message( 'Unable to rotate',ermsg );
      return;
    end;
  end;
  ...
  case 'hrfmax'
    ...
    if ( isempty( T ) )
      ...
      T = hrfmax(P, U, shapes, rotation_specs.defaults.iterations, load_state, pop);
      if isempty(T)
        return;
      end;
    end;
...
%      UR = apply_partitioned_to_matrix( Gheader, PR );
  ...
  case 'procrustes'
    if  isempty( rotation_specs.defaults.hrf_file) || isempty( rotation_specs.defaults.hrf_mat)
      ermsg = 'Unable to perform rotation as the target VR is not specified.';
      show_message( 'Unable to rotate',  ermsg );
      return;
    end;
