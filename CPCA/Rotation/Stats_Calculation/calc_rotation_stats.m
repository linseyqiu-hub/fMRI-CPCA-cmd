function [PR VR UR cvariance_rotated_tot] = calc_rotation_stats( P, U, V, T, nd, rotation_params, pop)
...
% --- N/A setPong(1)
  ...
  if ~isempty(pop)
    pop.setComment( 'Calculating Variances . . .' );
  end;
...
%     end;
  ...
  if ~isempty(pop)
    pop.setComment( 'Realigning Components . . .' );
  end;

  x = strfind( rotation_params.defaults.text, 'ALT' ) ;
  if ( rotation_params.defaults.oblique & ~isempty(x) )
    [cvariance_rotated_tot VR] = alt_component_variance( sumDiag, V, T );
    [aur_variance PR] = alt_component_variance( sumDiag, P, T );
    [ur vr pr cvariance_rotated_tot] = realign_rotated_components( UR, VR, P, cvariance_rotated_tot, pop );

  else
    cvariance_rotated_tot = component_variance( sumDiag, V, tsum );
    [ur vr pr cvariance_rotated_tot] = realign_rotated_components( U, V, P, cvariance_rotated_tot, pop );
  end
