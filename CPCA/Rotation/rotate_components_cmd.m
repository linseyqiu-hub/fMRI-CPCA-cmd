function [result, Zheader] = rotate_components_cmd(Zheader, scan_information, rotation_params, nd, log_fid, GAtyp, mask_registry )
...
%     Txt = sprintf( '%s Rotation %d components', upper( rotation_params.method ), nd );
    ...
    [T, PR, VR, UR] = compute_facs_cmd(Zheader, P, U, V, nd, tsum, nr, nc, Gheader, rotation_params, GAtyp );
    if ( isempty( T ) )       
      return;      
    ...
    [PR, VR, UR, cvariance_rotated_tot] = calc_rotation_stats_cmd(Zheader, scan_information, PR, UR, VR, T, nd, rotation_params );        
...
%    text_file = [ component_directory 'output_' txt_file ];
    ...
    text_file_header_cmd(Zheader, scan_information, nd, fid, log_fid, component_directory, text_file, rotation_params.Aindex, nvox )
    if strcmp( rotation_params.model, 'H' )
    ...
    pca_summary_cmd(Zheader,scan_information, sumDiag, ftext, cvariance_rotated_tot, fid, tsum );
    if ( fid ), fclose( fid );  end

    pca_summary_cmd(Zheader,scan_information, sumDiag, ftext, cvariance_rotated_tot, 1, tsum );
...
%     SubjectVector = [ 1:Zheader.num_subjects ];
    ...
    disp( 'Recomputing for beta check. . .' );
    ...
    if ~strcmp( rotation_params.prefix, 'H' )
      if ~strcmp( GAtyp, 'GAA' )
        ...
        betas_c_pos = calc_c_betas_cmd(Zheader, scan_information, fnm, Gheader, 1, 0, rotation_params.htype, mask_registry );

        % if ( ~isempty( funcs.clear_cache ) )  
        %   funcs.clear_cache(-1);    % -- force cache clearance priot to beta calc
        % end
        % funcs.memory_stats(); 
        betas_c_neg = calc_c_betas_cmd(Zheader, scan_information, fnm, Gheader, 0, 0, rotation_params.htype, mask_registry );
      end
    else

      disp( 'Positive Betas . . .' );
      betas_c_pos = calc_gmh_gc_betas_cmd(Zheader, scan_information, fnm, Gheader, Hheader, 1 );    
      disp( 'Negative Betas . . .' );
      betas_c_neg = calc_gmh_gc_betas_cmd(Zheader, scan_information,  fnm, Gheader, Hheader, 0 );
    ...
    if ~strcmp( GAtyp, 'GAA' )
      ...
      if ~strcmp( rotation_params.htype, 'GA' )
        ...
        output_HRF_cmd(Zheader, scan_information,  component_directory, out_file, PR, Gheader, nvox);
