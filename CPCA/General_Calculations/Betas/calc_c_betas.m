function betas = calc_c_betas( fn, Gheader, pos, pop, model, reg )
global Zheader scan_information

if ( nargin < 3 ) pos = 0; end;	% --- send pos = 1 to calculate positive values ---
if ( nargin < 4 ) pop = []; end;
if ( nargin < 5 ) model = 'G'; end;
if ( nargin < 6 ) reg = 0; end;
		...
		if ( size(x,1) > 1 ) % --= % ensure that there are loadings
			...
			for sn = 1:Zheader.num_subjects
				...
				if ~scan_information.isMulFreq	% --- different C retrieval fo meg and fMri sets
					retrieve_subject_C( GZheader, sn, '', 'Cs', model );
				else
					retrieve_full_subject_C( GZheader, sn, 'Cs' );
				end;
			...
			if strcmp( model, 'GA' )
				...
				for ii = 1:nconds
					sr = er + 1;
					er = sr + nbins - 1;
					ns = encodedCount( ii );   %% count of subjects with this condition encoded
