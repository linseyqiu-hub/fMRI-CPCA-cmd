function SoS = apply_partitioned_to_Z( funcs, Gheader, fid, pop, variable_creation)
...
global Zheader scan_information
  ...
  if scan_information.processing.model.process.apply_ga
    SoS = apply_GA_to_Z( funcs, Gheader, fid, pop );
    return
  end;

  if scan_information.processing.model.process.apply_gaa
    SoS = apply_GAA_to_Z( funcs, Gheader, fid, pop );
    return
  end;
  ...
  if variable_creation(1,2)
      create_residual_as_Z(model, pop, Gheader);
  end
  ...
  compile_CC_array( model, funcs, 0, pop );             % --- create whole brain CC matrix
  calculate_Eigenvalues( Gpath, 'CC', 0);

  if scan_information.mask.isRegistered % ---  && constant_define( 'PREFERENCES', 'general.gray_white_split' );
    compile_CC_array( model, funcs, 1, pop );           % --- create  gray matter CC matrix
    calculate_Eigenvalues( Gpath, 'CCG', 1);            % --- and calculate the eigenvector
    compile_CC_array( model, funcs, 2, pop );           % --- create white matter CC matrix
    calculate_Eigenvalues( Gpath, 'CCW', 2);            % --- and calculate the eigenvector
  end
  ...
  % --- called within a for each run loop, will calculate the GZ, C and B variables
    ...
    if isEncodedRun( SubjectNo, RunNo )
      ...
      [G gg] = load_run_G( Gheader, SubjectNo, RunNo );
      assignin( 'base', 'gg', gg);
  ...
  function calculate_GC_SD()
    ...
    if isEncodedRun( SubjectNo, RunNo )
      assignin( 'base', 'G', load_run_G( Gheader, SubjectNo, RunNo ) );
