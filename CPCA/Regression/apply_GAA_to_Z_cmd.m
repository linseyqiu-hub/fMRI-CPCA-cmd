function SoS = apply_GAA_to_Z_cmd(Gheader,Zheader, scan_information, fid )
...
%  prep_AA();
  ...
  for ii = 1:Zheader.num_subjects
    [Gs gg] = load_run_G_cmd(Zheader, Gheader, ii, 0);  % ---load original G
    GA = Gs * A;
  ...
  for SubjectNo = 1:Zheader.num_subjects
    ...
    Z = load_subject_Z_cmd(scan_information,SubjectNo,Zheader, ftag );


    disp( 'Loading Subject G . . .' );

    G = load_run_G_cmd(Zheader,Gheader, SubjectNo, 0);   % ---original G for full subject
  ...
  for SubjectNo = 1:Zheader.num_subjects
    ...
    for RunNo=1:Zheader.num_runs 
      ...
      if isEncodedRun_cmd( SubjectNo, RunNo,scan_information ) 
        ...
        disp( 'calculating and saving GnotAZ . . .' );
