function SoS = apply_GA_to_Z_cmd( Gheader,Zheader, scan_information, fid )
...
SoS = 0;
      ...
      for FrequencyNo=1:max(scan_information.frequencies, 1)
        ...
        for RunNo=1:Zheader.num_runs 
          if isEncodedRun_cmd( SubjectNo, RunNo, scan_information ) 
            [G gg] = load_run_G_cmd(Zheader, Gheader, SubjectNo, RunNo, 'GA');
            calculate_GZ();
          end
        ...
        for RunNo=1:Zheader.num_runs 
          if isEncodedRun_cmd( SubjectNo, RunNo, scan_information ) 
            G = load_run_G_cmd(Zheader, Gheader, SubjectNo, RunNo, 'GA');
  ...
  for psubj= 1:Zheader.num_subjects
    ...
    sid = subject_id_cmd( psubj,scan_information );

    % if ~isempty(pop)
    %   pop.setParticipant( psubj, size(SubjectVector,2), sid );
    % end;
    
    Primary = load_subject_B_cmd( Gheader, psubj,Zheader, '', model );
    ...
    if ( Zheader.num_subjects > 1 )
      ...
      if psubj < Zheader.num_subjects
        ...
        for ssubj = psubj+1:Zheader.num_subjects
          ...
          ssid = subject_id_cmd( ssubj,scan_information );
          Secondary = load_subject_B_cmd( Gheader, ssubj,Zheader, '', model );
...
%      eval( [ 'GZ_R' num2str(RunNo) ftag ' = G'' * Z;'] );
      ...
      if ( ~isempty(x) )
        ...
        fprintf( 'Possible timing vector error or corrupted data %s', str );
        SoS = 0;
        return;
      end
...
%       % -- B = gg * GZ   [ gg = sqrtm(inv( G' * G) ) ]
  ...
  function calculate_GC_SD()
    ...
    if isEncodedRun_cmd( SubjectNo, RunNo, scan_information ) 
      assignin( 'base', 'G', load_run_G_cmd( Zheader, Gheader, SubjectNo, RunNo, strcmp( model, 'GA') ) );
