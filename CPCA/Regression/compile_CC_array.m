function compile_CC_array( model, funcs, reg, pop )
...
global Zheader scan_information
  ...
  for SubjectNo = 1:Zheader.num_subjects - subject_minus
    ...
    if ~isempty(pop)
      ...
      pop.setIterations( iters.secondary_CC, pop.SECONDARY);
    end
    ...
    if isempty( ind )    
      ...
      for FrequencyNo = 1:max(scan_information.frequencies, 1)
        ...
        Primary( :,sbc:ebc) = load_subject_B( Gheader, SubjectNo, ftag, model );
      end
      ...
      for FrequencyNo = 1:max(scan_information.frequencies, 1)
        ftag = frequency_tag(FrequencyNo) ;
        B = load_subject_B( Gheader, SubjectNo, ftag, model );
        B = B(:, ind );
        Primary = [Primary B ];
      end
      clear B
      ...
      for SNo = SubjectNo+1:Zheader.num_subjects
        ...
        if isempty( ind )    
          ...
          for SubFrequency=1:max(scan_information.frequencies, 1)
            ...
            sbc = ebc + 1;
            ebc = sbc + Zheader.total_columns - 1;
            Secondary( :,sbc:ebc) = load_subject_B( Gheader, SNo, subtag, model );
          end  % --- each sub frequency
        else
          Secondary = [];
          for SubFrequency = 1:max(scan_information.frequencies, 1)
            subtag = frequency_tag(SubFrequency) ;
            B = load_subject_B( Gheader, SNo, subtag, model );
            B = B(:, ind );
