function [Zheader, scan_information] = create_residual_as_Z_cmd(Zheader, scan_information, model, Gheader )
  ...
  if ( nargin < 4 ) load( Zheader.Model.path, 'Gheader'); end
  ...
  for subjectNo = 1:Zheader.num_subjects
    ...
    sbj = subject_id_cmd( subjectNo , scan_information);
    ...
    for FrequencyNo = 1:max(scan_information.frequencies, 1)
      ...
      for RunNo = 1:size( Zheader.timeseries.subject(subjectNo).run, 1 )
        ...
        if isEncodedRun_cmd(subjectNo,RunNo,scan_information )
          ...
          GC = load_subject_GC_run_cmd( Gheader,Zheader, subjectNo, RunNo, ftag, model );
          Z = load_subject_run_Z_cmd(Zheader, scan_information, subjectNo, RunNo, ftag );
          E = Z - GC;
% ---Accum SSQ to /Residual/Z/Zn_vars
  ...
  Zheader.tsum = accumulate_Z_SSQ(Zheader,Residual_dir);
  save( mat_file, 'Zheader', '-append', '-v7.3');

  Z_SD_Report_cmd(Zheader, scan_information, [Residual_dir filesep ] );
...
%  mat_file = ['Residual' filesep '/ZInfo.mat'];
  ...
  %% --- Accumulate_Z_SSQ ()
  %  --- -----------------------------------
  function ts = accumulate_Z_SSQ(Zheader, Residual_dir)
