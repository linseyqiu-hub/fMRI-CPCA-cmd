function SSQ = normalize_subject( funcs, SubjectNo, FrequencyNo, fid, pop )
...
%   load( 'ZInfo.mat', 'Zheader', 'scan_information' );
  ...
  [ ftag fdsp ] = frequency_tag(FrequencyNo) ;
  ...
  if ~isempty(pop)
    ...
    pop.setRun( 1, num_encoded_runs( SubjectNo) );
  ...
  if ~isempty(pop)
    pop.setMessage( 'Standardizing scan images . . .' );
    iters = iteration_rule( 'Iterations', 'Subject Normalization', {'secondary'}, ...
       struct( 'Subj', 1, 'covar', perform_covariant_regression > 0) );
  ...
  for RunNo = 1:Zheader.num_runs;
    ...
    if isEncodedRun(SubjectNo, RunNo ) 
      ...
      if ~isempty(pop)
        pop.setRun( RunNo, num_encoded_runs( SubjectNo) );
        pop.setComment( 'Loading scan data . . .' );
      end;
      ...
      if ( perform_covariant_regression > 0 )  % -- apply any form of regression
        ...
        if ~isempty(pop)
          pop.setMessage(  'Standardizing prior to regression . . .' );
%          pop.setIterations( time_series, pop.SECONDARY );
        end;
        ...
        if ( scan_information.processing.subjects.process.movement_regress == 1 )
          ...
          if ( size( fil, 1) == 0 )
            str = sprintf( 'Unable to locate a movement regression text file for subject %s run %d', char(scan_information.SubjectID(SubjectNo)), RunNo ) ;
            show_message( 'File not found', str );
            SSQ = [];
            return
          end;
        ...
        if ~isempty(pop)
          pop.setMessage(  'Removing Trends . . .' );
        end;
...
%        sd = diag( Z( reg_data.ind(ii).zref) * Z(reg_data.ind(ii).zref)' );
      ...
      save( Zname, 'SD', 'tsum_removed', 'tsum_subject', '-v7.3', '-append' );
      save_columns_of_Z( Z, SubjectNo, RunNo, FrequencyNo );
      Z = [];
  ...
  save( 'ZInfo.mat', 'SD', 'tsum_removed', '-v7.3', '-append' );
  save_headers();
  
  save( Zvars, 'SD', 'tsum_removed', 'tsum_subject', 'SS_E', '-v7.3', '-append' );
