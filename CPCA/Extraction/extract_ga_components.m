function abort_proc = extract_ga_components( funcs, nd, log_fid, pop, GAtyp )
    ...
    if (  has_GC_var( GAheader, cc_var, GAtyp ) )		% BB file has been created
      ...
      if do_svd
        ...
        if m > 1  % --- use standard svd on scalars
          C_Eigenvalues = load_GC_var( Gheader, 'C_Eigenvalues', GAtyp );
...
%          load( [ cc_file '/GCC.mat'], 'C_Eigenvalues', 'CC' );
      ...
      for SubjectNo=1:Zheader.num_subjects
        ...
        retrieve_subject_G( Gheader, SubjectNo, 0, 'GA' );
        gg = sqrtm(pinv( G'*G ) );
        ...
        for FrequencyNo=1:max(scan_information.frequencies, 1)
          ...
          B = [B load_subject_B( Gheader, SubjectNo, ftag, GAtyp ) ];
        end;
      ...
      cvariance_unrotated_tot = component_variance( Aheader.model( Aheader.Aindex).sd(1), V );
      ...
      pca_summary( Aheader.model( Aheader.Aindex).sd(1), 'GA', cvariance_unrotated_tot, fid );
      ...
      pca_summary( Aheader.model( Aheader.Aindex).sd(1), 'GA', cvariance_unrotated_tot, 1 );
      ...
      funcs.memory_stats(); 
      betas_c_pos = calc_c_betas( save_file, Gheader, 1, 0 , GAtyp );
      ...
      funcs.memory_stats(); 
      betas_c_neg = calc_c_betas( save_file, Gheader, 0, 0, GAtyp );
      ...
      if (fid)
        ...
        for SubjectNo = 1:Zheader.num_subjects
          ...
          sid = subject_id( SubjectNo );
          GCsd = load_subject_GC_var( Gheader, SubjectNo, 'subject_GCsd', GAtyp );
