function abort_proc = extract_g_subject_components_cmd( Zheader, scan_information , nd, log_fid, mask_registry )
...
if ( has_dir )
    ...
    for SubjectNo = 1:Zheader.num_subjects
        ...
        for FrequencyNo=1:max(scan_information.frequencies, 1)
            ...
            retrieve_subject_GC_cmd(Zheader, Gheader, SubjectNo, ftag );
            if ~isempty( ind )
                GC = GC(:,ind);
            end
            %
        ...
        disp( 'Variance and thresholds' );

        VR = [];
        eval( [' VR = alt_VR_S' num2str(SubjectNo) ';' ] );
        ep = calc_ext_Pos_Neg(VR');
        eval( [' ep_' num2str(SubjectNo) ' = ep;' ] );
        cvariance_rotated_tot = component_variance_cmd(Zheader, sumDiag, VR' );

        save( [outdir out_mat], ['alt_VR_S' num2str(SubjectNo) ], ['alt_VR_coeff_S' num2str(SubjectNo) ], ['ep_' num2str(SubjectNo) ], 'cvariance*', '-append');
        save( [subj_dir out_mat], 'ep', 'VR', 'cvariance*', '-append');
        ...
        text_file_header_cmd(Zheader, scan_information, nd, fid, 0, component_directory, text_file, 0, nvox );
        pca_summary_cmd(Zheader,scan_information,sumDiag, ['GC' constant_define( 'REGISTRATION_SUMMARY_TAG', mask_registry )], cvariance_rotated_tot, fid, tsum );
        ...
        g_images_unrotated_alt_vr_cmd(Zheader, scan_information, nd, SubjectNo, log_fid, mask_registry );

    end   % --- each subject

    eval ( [ 'save( ''' outdir out_mat ''', ''VR_*'', ''-append'');' ] );
    ...
    text_file_header_cmd(Zheader, scan_information, nd, fid, 0, outdir, vals_output, 0, nvox );
    print_subject_cov_cmd(Zheader,scan_information, fid, VR_ss_cov, VR_var_cov, Gheader );
    print_subject_cov_cmd(Zheader,scan_information, fid, VR_ss_coef, VR_var_coef, Gheader, 'corrcoef' );
