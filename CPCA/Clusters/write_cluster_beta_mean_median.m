function write_cluster_beta_mean_median( rotation_params, Gheader, nd, pop, mask_registry )
...
%       abs_cluster_no = 0;
      ...
      if is_active_threshold(thresh_index)
        ...
        if ~isempty(pop)
          pop.setComment( ' Positive Clusters . . .' );
        end;
...
%               vr_comp = vr_cmp(x);
              ...
              if ( size(x,1) > 1 )		% ensure that there are loadings 
                ...
                if ~isempty(pop)
                  pop.setComment( 'Positive Clusters . . . Writing . . .' );
                end;
        ...
        if ~isempty(pop)
          pop.increment( pop.SECONDARY );
          pop.setComment( ' Negative Clusters . . .' );
        end;
...
%               vr_comp = vr_cmp(x);
              ...
              if ( size(x,1) > 1 )		% ensure that there are loadings 
                ...
                if ~isempty(pop)
                  pop.setComment( 'Negative Clusters . . . Writing . . .' );
                end;
