function mean_beta_images( Gheader, pop, log_fid )
...
global Zheader scan_information 
  ...
  if ( has_dir )
    ...
    if ~isempty(pop)
      pop.setMessage( 'Creating mean beta images . . .' );
    end;
    ...
    for SubjectNo = 1:Zheader.num_subjects
      ...
      for FrequencyNo=1:max(scan_information.frequencies, 1)
        ...
        C = [C load_subject_C( Gheader, SubjectNo, ftag ) ];
      end;
      ...
      for condition = 1:Gheader.conditions
        if isEncoded( SubjectNo, condition )
          sr = er + 1;
