function g_images_unrotated_cmd(Zheader, scan_information, nd, log_fid, GAtyp, mask_registry  )
...
if ( has_dir )
    ...
    for FrequencyNo = 1:max(scan_information.frequencies,1)
        ...
        for component_no = 1:nd % --=
            ...
            threshold_tops_cmd(Zheader,scan_information,displays, nd, component_directory, Aidx, ...
    ...
    save( [component_directory in_file], 'component_loadings', '-append' );

    if ~scan_information.isMulFreq	% --- bypass cluster data on meg data for now
        save( [image_directory load_file], 'MNI', 'mask_registry', '-append' );
    end
