function recreate_g_images_cmd( Zheader, scan_information,rotation_params, nd, comps_to_flip, GAtyp, mask_registry )
...
% --- nd * max(scan_information.frequencies, 1)
  ...
  if nargin < 5,  comps_to_flip = 0;  end
      ...
      if ~(scan_information.isMulFreq == 1)
          for thr = 1:num_global_thresholds()
              if is_active_threshold(thr)
                fid = files(thr);
                threshold_print_mni_cmd(0,component_no,fid,displays,thresholds,MNI.component(component_no), thr);                  
              end
          end
      end
