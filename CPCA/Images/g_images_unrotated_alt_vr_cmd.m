function g_images_unrotated_alt_vr_cmd(Zheader, scan_information, nd, subjectNo, log_fid, mask_registry )
...
%    load_file = fs_filename( 'loadings', 'G', 'unrotated', [] );
    ...
    for FrequencyNo=1:max(scan_information.frequencies, 1)
      ...
      for component_no = 1:nd
        ...
        fprintf('%s Creating Images for Subject\n', Sts);
        ...
        fid = fopen( [mni_directory MNI_out], mode );
         ...
         if ( component_no == 1 )
           text_file_header_cmd(Zheader, scan_information,  nd, fid, 0, component_directory, MNI_out, 0, nvox );
%           pca_summary( sumDiag, ['GC' constant_define( 'REGISTRATION_SUMMARY_TAG', mask_registry)], cvariance_rotated_tot, fid, tsum );
        ...
        if ~(scan_information.isMulFreq == 1)	% --- bypass cluster data on meg data for now
          ...
          for thr = 1:size( ep(1).percentiles, 1 )
            ...
            if is_active_threshold(thr)
              ...
              fprintf('%s Clusters @ %s %\n',Sts, num2str(ep(1).percentiles(thr).cutoff) );
    ...
    save( [component_directory VR_file], 'component_loadings', 'VR', 'ep', '-append' );

    if ~(scan_information.isMulFreq == 1)	% --- bypass cluster data on meg data for now
      save( load_file, 'MNI', '-append' );
      ...
      if mask_registry == 0   % --- cluster info only on whole brain for now
        ...
        write_subject_cluster_data_cmd(Zheader,scan_information, parms, VR, MNI, subjectNo );
      end
