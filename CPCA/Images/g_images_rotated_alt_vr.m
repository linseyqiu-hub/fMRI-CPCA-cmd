function g_images_rotated_alt_vr( rotation_params, subjectNo, nd, log_fid, pop, mask_registry )
      ...
      for component_no = 1:nd
       ...
       loadings = [];
        ...
        fid = fopen( [component_directory MNI_out], mode );
         ...
         if ( component_no == 1 )
           text_file_header( nd, fid, 0, component_directory, MNI_out, rotation_params.Aindex, nvox );
        ...
        if ~isMultiFrequency()	% --- bypass cluster data on meg data for now
          ...
          for thr = 1:size( ep(1).percentiles, 1 )
            ...
            if is_active_threshold(thr)
              ...
              if ~isempty(pop)
               pop.setComment(  [Sts ' Clusters @ ' num2str(ep(component_no).percentiles(thr).cutoff) '%'] );
              end;
    ...
    if ~isMultiFrequency()	% --- bypass cluster data on meg data for now
      ...
      if mask_registry == 0   % --- cluster info only on whole brain for now
        ...
        if  constant_define( 'PREFERENCES', 'cluster.calculate_mean' , 0 ) || ...
          ...
          write_subject_cluster_data( rotation_params, VR, MNI, subjectNo, pop );
        end
      end
    end;
