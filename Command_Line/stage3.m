...
try
    ...
    Extract_Rotate_Components(state.config.baseDIR, state.config.num_components, 'E', 'G');
    fprintf('   Completed: %d components extracted.\n', state.config.num_components);
 
    % Step 2: Rotate components (if specified)
    if isfield(state.config, 'rotation_method') && ~isempty(state.config.rotation_method)
        fprintf('\n2. Rotating components using %s...\n', state.config.rotation_method);
        rot_method = {state.config.rotation_method};
        Extract_Rotate_Components(state.config.baseDIR, state.config.num_components, 'R', 'G', rot_method);
