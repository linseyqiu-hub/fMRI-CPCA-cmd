...
% ---
% --- input:
% ---     base_dir: specify work folder (where scan data lives)
% ---     fileName: 
...
% ---     output_dir: directory where output files (ZInfo.mat, mask, etc.) will be written
% ---                 if empty, defaults to base_dir (legacy behaviour)
%---
% --- output:
% ---   Zinfo.mat:
% ---     scan_information             
...
cd(output_dir);   % cd to base_dir so scan data is accessible

% ---- check inputs --------------------------------
% files.txt lives in output_dir
if exist(fullfile(output_dir, p.Results.fileName), 'file') ~= 2
...
% confirm the directory pointed to still exists
...
cd(output_dir);
scan_information.mask.file = p.Results.maskName;
x = fullfile(base_dir, scan_information.mask.file);   % mask lives in base_dir (input data)
if ~isempty(p.Results.maskMethod)
    x = fullfile(output_dir, scan_information.mask.file);
else
    x = fullfile(base_dir, scan_information.mask.file);
end

fprintf('DEBUG mask path: %s\n', x);
fprintf('DEBUG spm_existfile: %d\n', spm_existfile(x));
fprintf('DEBUG exist(): %d\n', exist(x, 'file'));
if ( spm_existfile(x) > 0 )
    ...
    if ~isRegistered(scan_information.mask)
        ...
        % cpca_read_vol always looks in baseDIR internally
        % copy registered_mask to baseDIR so it can find it
        copyfile(fullfile(output_dir, 'registered_mask.img'), fullfile(base_dir, 'registered_mask.img'));
        copyfile(fullfile(output_dir, 'registered_mask.hdr'), fullfile(base_dir, 'registered_mask.hdr'));
    
        msk = cpca_read_vol(fullfile(base_dir, 'registered_mask.img'));
    
        % clean up copied files from baseDIR
        delete(fullfile(base_dir, 'registered_mask.img'));
        delete(fullfile(base_dir, 'registered_mask.hdr'));
    ...
    get_size = @(hdr) prod(hdr.dim(2:min(4,end)));

    if isfield(scan_information, 'raw_data') && ...
    isfield(scan_information.raw_data, 'header') && ...
    isfield(scan_information.raw_data.header, 'dim') && ...
    isfield(scan_information.mask, 'header') && ...
    isfield(scan_information.mask.header, 'dim')

        x = get_size(scan_information.mask.header) == ...
            get_size(scan_information.raw_data.header);

        y = prod(scan_information.mask.header.pixdim(2:4)) == ...
            prod(scan_information.raw_data.header.pixdim(2:4));
    ...
    fprintf('DEBUG mask header fields: %s\n', strjoin(fieldnames(scan_information.mask.header), ', '));
    fprintf('DEBUG mask ind size: %d\n', numel(scan_information.mask.ind));
    ...
    if ( isempty( Zheader.Z_Directory ) ) && prod( size( scan_information.mask.ind ) ) > 1
        Zheader.Z_Directory = [ output_dir filesep ];   % output goes to output_dir
    end
    ...
    save_Zinfo( Zheader, scan_information)   % pass output_dir to save_Zinfo

end

disp('Verifying the mask...')
cd(base_dir);
mask_verification_cmd(Zheader, scan_information)   % pass output_dir
cd(output_dir);
disp('Done!')
