function text_file_header_cmd(Zheader, scan_information, nd, fid, log_fid, base_path, filename, withA, nvox, thr)
% --- pass Aheader.Aindex value as index to use

  if nargin < 7,        filename = '';  end
  if nargin < 8,        withA = 0;      end
  if nargin < 9,        nvox = Zheader.total_columns;      end
  if nargin < 10,        thr = 0;    end
