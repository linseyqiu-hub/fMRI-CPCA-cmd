function pca_summary_cmd(Zheader,scan_information, sumDiag, model, S, fid, tsums )

isROI = strcmp( model, 'GCr' );

if nargin < 7,   tsums = Zheader.tsum;  end
...
if Zheader.num_Z_arrays > 0
  if strcmp( model, 'GC' )
    ...
    SSQ = load_GC_var_cmd(Gheader, Zheader,  'SSQ' );
  ...
  if ~isempty( SSQ )
    ...
    for freq = 1:max( 1, Zheader.num_Z_arrays )
      ...
      ftag = frequency_tag_cmd( freq,scan_information );
      ftag = strrep( ftag, '_', '' );
