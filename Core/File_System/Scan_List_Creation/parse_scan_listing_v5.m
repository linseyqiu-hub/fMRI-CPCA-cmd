function parse_scan_listing_v5( filename ) 
...
global scan_information 
  ...
  pop.setMessage( 'Processing file list . . .' );
  pop.show();
      ...
      if ( input_line(1) ~= '#' ) && ( input_line(1) ~= '%' )
        ...
        switch e(idx_tag).var
          ...
          case 'subjects'         
            ...
            pop.setIterations(NumSubjects, pop.PRIMARY );          
%            SubjectNo = SubjectNo + 1;
         ...
         case 'frlabels'     
          ...
          case 'scandir'     % scandir:s06/images45Hz id:s06 group:<na> frequency:images45Hz runs:runa~runb
            ...
            pop.setParticipant(SubjectNo, NumSubjects, '' );
...
%                scan_information.freq_names = [ scan_information.freq_names { e(idx_frequency).value} ]; 
  ...
  pop.setMessage( 'checking RP and timing files . . .' );

  pop.setIterations(scan_information.NumSubjects, pop.PRIMARY );
  for SubjectNo = 1:scan_information.NumSubjects
    pop.setParticipant(SubjectNo, scan_information.NumSubjects, '' );
