function scan_information = parse_scan_listing_cmd_v5( filename, scan_information)
        ...
        if ( input_line(1) ~= '#' ) && ( input_line(1) ~= '%' )
            ...
            switch e(idx_tag).var
                ...
                case 'scandir'     % scandir:s06/images45Hz id:s06 group:<na> frequency:images45Hz runs:runa~runb
                    ...
                    if ( this_defined_freq > scan_information.frequencies ) 
                        this_defined_freq = 1; 
                    end
                    ...
                    if this_defined_freq == 1
                        ...
                        if any(x)  
                            scan_information.duplicate_IDs = 1;  
                        end  	% [2.96]
...
if isempty( scan_information.freq_names )
    scan_information.freq_names = {''};
end
if isempty( scan_information.freq_dirs )
    scan_information.freq_dirs = {''};
end
if isempty( scan_information.SubjDir )
    scan_information.SubjDir = scan_information.SubjectDirs';
end
if scan_information.isMulFreq
    scan_information.NumSubjects = scan_information.NumSubjects /  scan_information.frequencies;
end

disp( 'checking RP and timing files . . .' );
