function retrieve_full_subject_C_cmd(Zheader, scan_information, Gheader, SubjectNo, varname )
...
% --- for full non frequency subject data use retrieve_subject_C 
  ...
  ftag = frequency_tag_cmd(1,scan_information) ;              % --- only used to test for C existence
  ...
  if ~isempty(n)
    ...
    for fno = 1:max( scan_information.frequencies, 1)
      ...
      ftag = frequency_tag_cmd(fno,scan_information) ;  
